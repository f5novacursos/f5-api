...
// ── Ciclo de vida da instância (Configurações → WhatsApp) ──
// Instância única e fixa (zapf5cursos) — sem multi-tenant, ao contrário do F5 Social.

/** QR code atual da instância (base64) para o admin escanear. */
async function obterQrCode() {
  const dados = await evoFetch(`/instance/connect/${INSTANCE}`);
  return dados.base64 || dados.qrcode?.base64 || dados.code || null;
}

/** Estado da conexão: 'open' = conectado e pronto para enviar. */
async function estadoConexao() {
  const dados = await evoFetch(`/instance/connectionState/${INSTANCE}`);
  return dados.instance?.state || dados.state || 'close';
}

/** Desconecta o número (o admin pode reconectar depois lendo o QR de novo). */
async function desconectarInstancia() {
  return evoFetch(`/instance/logout/${INSTANCE}`, { method: 'DELETE' });
}

module.exports = {
  criarGrupo, listarGrupos, obterConviteGrupo, enviarTexto, enviarMidia,
  obterQrCode, estadoConexao, desconectarInstancia,
};
