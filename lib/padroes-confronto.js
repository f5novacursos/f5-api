...
// Slots por liga — igual ao worker — para arredondar slotMin da EasyCo
const LIGA_SLOTS = {
  ...
  sul_americana:  [0,3,6,9,12,15,18,21,24,27,30,33,36,39,42,45,48,51,54,57],
  premier_league: [1,4,7,10,13,16,19,22,25,28,31,34,37,40,43,46,49,52,55,58],
  // Betano
...
function nearestSlot(liga, minuto) {
  const slots = LIGA_SLOTS[liga];
  if (!slots) return minuto;
  return slots.reduce((best, s) => Math.abs(s - minuto) < Math.abs(best - minuto) ? s : best, slots[0]);
}
...
module.exports = function(db, tabela, ligasCfg) {
  ...
  return async function(req, res) {
    try {
      ...
      // ── 2. Monta índice rápido: "liga|teamA|teamB" → stats ─────────
      const idx = new Map();
      for (const r of stats) {
        idx.set(`${r.liga}|${r.team_a}|${r.team_b}`, r);
      }
      ...
      await Promise.all(ligas.map(async ({ sub, liga, provider = 'bet365' }) => {
        try {
          ...
          for (const j of jogos) {
            ...
            const rawDate = j.startTime || j.start_time || j.date;
            if (!rawDate) continue;
            ...
            const slotMin = j.slotMin ?? j.slot_min ?? nearestSlot(liga, new Date(rawDate).getUTCMinutes());
      ...
      const resultado = {
        ...
        previsoes,
        _debug_idx_sample: [...idx.keys()].slice(0, 5),
        _debug_pendentes_sample: pendentes.slice(0, 5).map(p => `${p.liga}|${p.teamA}|${p.teamB}`),
      };
