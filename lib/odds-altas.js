...
const PISO = {
  '0-0':    25,  // base ~10% — acima de 25% já tem edge forte
  '5+':     15,  // base ~9%  — acima de 15% já é surpreendente
  'total1': 30,  // 1 gol exato — base ~22%
  'total2': 35,  // 2 gols exato — base ~26%
  'total3': 25,  // 3 gols exato — base ~20%
  'total4': 15,  // 4 gols exato — base ~12%
};
...
module.exports = function(db, tabela, ligasCfg, clockOffsetMs = -10800000) {
  return async function(req, res) {
    try {
      ...
      const ligaWhere = ligaFiltro ? `AND liga = $3` : '';
      const params = [horas, minAmostra];
      ...
      const [{ rows: stats }, { rows: historico }] = await Promise.all([
        db.query(sqlStats, params),
        db.query(sqlHist, params),
      ]);
      ...
      const resultado = { ok: true, total: previsoes.length, pendentes_encontrados: pendentes.length, previsoes };
