...
// 🚫 VETO por provedor+mercado — veredito do paper-trading (7 dias, 08/07/2026,
// ~24,7k flags conferidas nas duas casas). Mercados cujo EV REALIZADO ficou
// abaixo de 1,00 com amostra, coerente com a calibração estática de 03/07:
//   betano|gol5: 0,51 (4 greens em 111) — estimativa infla em confronto raro
//   betano|ams:  0,93 (n=367) — calibração já dizia 0,95-0,99 (nunca paga)
//   bet365|gol1: 0,94 (n=667)
//   bet365|gol3: 0,91 (n=372)
// Vetado nunca acende como value nem entra no tracker (mercados[].veto=true e
// regua marca veto). O histórico antigo fica na tabela — reavaliar mensalmente
// via GET /odds-altas/historico antes de soltar um veto.
const VETO_MERCADO = {
  betano: new Set(['gol5', 'ams']),
  bet365: new Set(['gol1', 'gol3']),
};
...
async function calcularValue(db, tabela, ligasCfg, horas, evMin, toRealUtcMs, clockOffsetMs) {
  ...
  const vetoSet = VETO_MERCADO[provedor] || new Set();
  ...
  for (const p of pendentes) {
    ...
    for (const m of MERCADOS) {
      ...
      const ev = odd * f;
      const vetado = vetoSet.has(m.odd);
      mercados.push({
