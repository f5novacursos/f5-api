...
module.exports = function (db, tabela) {
  ...
  return async (req, res, next) => {
    try {
      ...
      // piso de amostra do 3-tiro: não declarar padrão com 6/6 (overfit + comparações múltiplas).
      // o front manda 5; aqui forço >= 12 independente do que vier.
      const minAm3 = Math.max(12, Number(min_amostra) || 0);
      // edge mínimo do 1º TIRO: o gatilho tem que superar a base do mercado em pelo menos isto.
      // sem isto, "100% em 3 tiros" é só a cumulação do Martingale inflando mercado de base alta
      // (OVER 1.5 base ~72% → edge de 3 tiros sempre parece bom, mas o 1º tiro não prevê nada).
      const MIN_EDGE1 = 8;
      // Máx. entradas por liga — evita inundar a tela com uma liga dominante.
      // Segue a mesma filosofia do top=3 da aba Especial: poucos, selecionados.
      const MAX_POR_LIGA = 3;
      // Piso de confiança (1º tiro) por liga+mercado — portado da aba Especial 25/06/2026.
      // Calibrado com 3966 entradas conferidas: combinações onde subir o piso global não ajuda
      // ou onde o acerto é estruturalmente baixo independente da confiança.
      //   VETADOS (999): premier_league|OVER 2.5 (59%), copa_america|OVER 2.5 (76%/piora),
      //   euro_cup|OVER 2.5 (73%/piora), brasileirao|AMBAS SIM (65%), premier_league|AMBAS SIM (75%),
      //   sul_americana|AMBAS SIM (71%).
      //   Piso elevado: copa_estrelas|OVER 2.5 →70 (100% em >=70), sul_americana|OVER 2.5 →70 (89%),
      ...
      // Só os 2 mercados de base alta (mesmo corte da aba Especial — 26/06/2026).
      // Futebol virtual é RNG: as 8 geometrias não preveem (edge ilusório — prova out-of-sample:
      // 90% prometido vira 56% real). Cortar OVER 2.5/AMBAS/OVER 3.5/0-0/UNDER 1.5 (base baixa,
      // viravam os reds) e ofertar só OVER 1.5 (~68%) e UNDER 2.5 (~58%), que o Martingale segura.
      const MERCADOS_OFERTADOS = ['OVER 1.5', 'UNDER 2.5'];
          ...
          for (let si = 0; si < slotList.length; si++) {
            for (const [gName, G] of Object.entries(GEOMS)) {
              ...
              for (const [dim, fn] of Object.entries(CONDS)) {
                ...
                if (!st || st.total3 < minAm3) {
                  st = acc[`${gName}|${dim}|*|${condKey}`];
                  slotEspecifico = false;
                }
                if (!st || st.total3 < minAm3) continue;

                let best = null;
                for (const m of MERCADOS_OFERTADOS) {
                  ...
                  const pisoLM = PISO_LIGA_MERCADO[`${ligaKey}|${m}`] || 0;
                  if (c1 < pisoLM) continue; // veto/piso por liga+mercado (calibrado 25/06/2026)
                  const e1 = c1 - base(m);    // edge do 1º tiro = sinal REAL, antes da inflação do Martingale
                  const e3 = c3 - base3(m);   // edge cumulativo em 3 tiros
                  if (e1 < MIN_EDGE1) continue;  // mata OVER 1.5/AMBAS que só "acertam" pela base alta
                  if (e3 <= 0) continue;
                  if (!best || e1 > best.e1 || (e1 === best.e1 && e3 > best.e3)) best = { m, c1, c2, c3, e1, e3 };
                }
                if (!best) continue;
      ...
      const byKey = {};
      const melhorQue = (a, b) => a.edge !== b.edge ? a.edge > b.edge : (a.edge3 !== b.edge3 ? a.edge3 > b.edge3 : a.amostra3 > b.amostra3);
      ...
      const ordenados = Object.values(byKey)
        .sort((a, b) => (b.edge - a.edge) || (b.edge3 - a.edge3) || (b.amostra3 - a.amostra3));
      ...
      for (const p of ordenados) {
        ...
        // Cap por liga: máx MAX_POR_LIGA entradas (mesma filosofia do top=3 da Especial)
        if ((porLigaCount[p.liga] || 0) >= MAX_POR_LIGA) continue;

        // Ocupa todos os jogos desta sequência
        tentKeys.forEach(key => ocupados.add(key));
        porLigaCount[p.liga] = (porLigaCount[p.liga] || 0) + 1;
